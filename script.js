...
function mainLoop(){
  ...
  for(let j=0;j<numRings;j++){
    ...
    for(let i=0;i<numCorners;i++){
      ...
      dotOriginPos.push(new POS(orginX,orginY,orginZ));
    }
  }

  for(let i=0;i<numRings*numCorners;i++){
  ...
  let tama = [];
  for(let i= 0;i<numCorners*numCorners/2;i++){
    let color = "white"
    if(i%2 == 0){
      color = "blue"
    ...
    let  j = i + numCorners;
    let p = i+numCorners+1; 
    if(p % numCorners == 0) p -= numCorners;
    let k = i + 1;
    if(k % numCorners == 0) k -= numCorners;
    tama.push(new Polygon(dotMovePos[i],dotMovePos[j],dotMovePos[p],dotMovePos[k],4,color));
  }
  ...
  let paintcounter = 0;
    ...
    for(let i=0;i<tama.length;i++){
      if(!(tama[i].isdraw) && tama[i].ZPosition>=tempZposition){
        tempZposition = tama[i].ZPosition;
      }
    }
    for(let i=0;i<tama.length;i++){
      if(!(tama[i].isdraw) && tama[i].ZPosition>=tempZposition){
...
function draw(cubes){
  ...
  //上面
  Polygones.push(new Polygon(cube.dotMovePos[3],cube.dotMovePos[7],cube.dotMovePos[4],cube.dotMovePos[0],4,"rgb(255,255,0)"));
  //右側面
  Polygones.push(new Polygon(cube.dotMovePos[3],cube.dotMovePos[2],cube.dotMovePos[6],cube.dotMovePos[7],4,"rgb(160,200,255)"));
  //下面
  Polygones.push(new Polygon(cube.dotMovePos[1],cube.dotMovePos[5],cube.dotMovePos[6],cube.dotMovePos[2],4,"rgb(255,255,255)"));
  });
  while(true){
