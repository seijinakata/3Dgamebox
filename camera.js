...
let extensionMatrix = new Matrix();
extensionMatrix.mulScaling(SCREEN_SIZE_W,SCREEN_SIZE_H,1);

let offsetMatrix = new Matrix();
offsetMatrix.mulTranslate(SCREEN_SIZE_W/2,SCREEN_SIZE_H/2,0);
let CRotX = 0,
    CRotY = 0,
    CRotZ = 0,
...
class Polygon{
  ...
  constructor(Pos1,Pos2,Pos3,UV,image){
    ...
    this.crossVector = culVecCross(Pos1,Pos2,Pos3);
  }
...
function culVecCross(ver1,ver2,ver3){
  ...
  let N_x = (ver1.y-ver2.y)*(ver2.z-ver1.z)-(ver1.z-ver2.z)*(ver3.y-ver2.y);
  let N_y = (ver1.z-ver2.z)*(ver3.x-ver2.x)-(ver1.x-ver2.x)*(ver3.z-ver2.z);
  let N_z = (ver1.x-ver2.x)*(ver3.y-ver2.y)-(ver1.y-ver2.y)*(ver3.x-ver2.x);

  let length = Math.sqrt(N_x * N_x + N_y * N_y + N_z * N_z);
  crossVector3.x =  N_x /= length;
  crossVector3.y =  N_y /= length;
  crossVector3.z =  N_z /= length;
