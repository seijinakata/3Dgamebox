...
class Polygon{
  ...
  draw(renderZBuffer,objectZ){
    if(this.UV.isup == true){
    	triangleToBuffer(renderZBuffer,objectZ,1,this.image,
	        [
...
cubeImage.addEventListener("load", function() {
	...
	cubes.push(new Object(orgCubeVerts,0.0,-0.27,3,0,0,0,0,false,true,cubePixelImage));
	cubes.push(new Object(orgCubeVerts,-0.6,-0.27,4.9,0,0,0,0,false,true,cubePixelImage));
	cubes.push(new Object(orgCubeVerts,-0.6,-0.77,5.0,0,0,0,0,false,true,cubePixelImage));
	cubes.push(new Object(orgCubeVerts,0.6,-0.77,5,0,0,0,0,false,true,cubePixelImage));
	cubes.push(new Object(orgCubeVerts,0.6,-0.27,4.9,0,0,0,0,false,true,cubePixelImage));
	...
	for(let i=0;i<moveObjects[j].polygonNum;i++){
	  ...
	  if(moveObjects[j].backCullingFlag == true){
	    if(moveObjects[j].polygonList[i].crossZ<0){
			let objectZ = moveObjects[j].centerObjZ;
		  	moveObjects[j].polygonList[i].draw(renderZBuffer,objectZ);
	    } 
	  }else{
			let objectZ = moveObjects[j].centerObjZ;
		  	moveObjects[j].polygonList[i].draw(renderZBuffer,objectZ);
