...
function objectPolygonPush(objects,objectNumber,moveObjects,viewMatrix){
  ...
  for (var i = 0; i < object.verts.length; i++) {
    ...
    /*
    worldVerts[i] = matVecMul(inverseViewPortMatrix,worldVerts[i]);
    worldVerts[i] = matVecMul(inverseProjectionMatrix,worldVerts[i]);
    worldVerts[i] = matVecMul(inverseViewMatrix,worldVerts[i]);
    //逆行列をかけることでworldに戻せる。シャドウマップに使えるかも
    //console.log(worldVerts[i])
    worldVerts[i] = matVecMul(viewMatrix,worldVerts[i]);
    worldVerts[i] = matVecMul(projectionMatrix,worldVerts[i]);    
    worldVerts[i] = matVecMul(viewPortMatrix,worldVerts[i]);
...
monkeysImage.addEventListener("load", function() {
	...
	let uv = [];
	let faceUV = [];
	monkeys.push(new Object(monkeyVerts,0.0,-0.6,1,-180,0,0,0.5,0,faceIndex,uv,faceUV,false,true,monkeyPixelImage));
