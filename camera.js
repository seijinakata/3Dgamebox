...
monkeysImage.addEventListener("load", function() {
	...
	let uv = [];
	let faceUV = [];
	monkeys.push(new Object(monkeyVerts,0.0,-0.6,1.5,180,0,0,1,1,1,0,faceIndex,uv,faceUV,false,true,monkeyPixelImage));
...
for(let j=0;j<moveObjects.length;j++){
	for(let i=0;i<moveObjects[j].polygonNum;i++){
	  ...
	  if(moveObjects[j].orgObject.backCullingFlag == true){
       ...
       triangleToBuffer(shadowMap,null,1,moveObjects[j].polygonList[i].image,moveObjects[j].polygonList[i].moveVertices,
          [
...
for(let j=0;j<moveObjects.length;j++){
	for(let i=0;i<moveObjects[j].polygonNum;i++){
	  ...
	  if(moveObjects[j].orgObject.backCullingFlag == true){
	    if(moveObjects[j].polygonList[i].crossVector3[2]<0){
        triangleToBuffer(zBuffering,shadowMap,1,moveObjects[j].polygonList[i].image,moveObjects[j].polygonList[i].moveVertices,
          [
            moveObjects[j].polygonList[i].UV[0], moveObjects[j].polygonList[i].UV[1],
            moveObjects[j].polygonList[i].UV[2], moveObjects[j].polygonList[i].UV[3],
            moveObjects[j].polygonList[i].UV[4], moveObjects[j].polygonList[i].UV[5]
