...
if( skyPixelImage.length == 0  || cubePixelImage.length == 0 ||
  ...
  for(let i = spheres.length;i<cubes.length;i++){
  	let currentObject = moveObjects[i];
  	for(let j = spheres.length;j<cubes.length+planes.length;j++){
  	  if(AABBcollision(currentObject,moveObjects[j]) == true && (currentObject.orgObject.centerObjY<moveObjects[j].orgObject.centerObjY)){
  ...
  for(let i = spheres.length;i<cubes.length;i++){
  	if(moveObjects[0].gravityCollision == false){
