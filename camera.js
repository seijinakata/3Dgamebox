...
import {SCREEN_SIZE_H,SCREEN_SIZE_W,renderBuffer,pixel,bufferPixelInit,bufferInit,pictureToPixelMap,
  dotPaint,dotLineBufferRegister,triangleRasterize,textureTransform,triangleToBuffer} from './paint.mjs';
...
class Matrix{
    ...
    this._13 = this._12 * -s + this._13 * c;
    this._12 = tmp;
    //2行目
    tmp = this._22 * c + this._23 * s;
    this._23 = this._22 * -s + this._23 * c;
    this._22 = tmp;
    //3行目
    tmp = this._32 * c + this._33 * s;
    this._33 = this._32 * -s + this._33 * c;
    this._32 = tmp;
	}
	mulRotateY(r) {
    let r360 = r*Math.PI/180;
  ...
  MatMul(m) {
    ...
    tmp._11 = this._11 * m._11 + this._12 * m._21 + this._13 * m._31 +this._14 * m._41;
    tmp._21 = this._21 * m._11 + this._22 * m._21 + this._23 * m._31 +this._24 * m._41;
    tmp._31 = this._31 * m._11 + this._32 * m._21 + this._33 * m._31 +this._34 * m._41;
    tmp._41 = this._41 * m._11 + this._42 * m._21 + this._43 * m._31 +this._44 * m._41;

    tmp._12 = this._11 * m._12 + this._12 * m._22 + this._13 * m._32 +this._14 * m._42;
    tmp._22 = this._21 * m._12 + this._22 * m._22 + this._23 * m._32 +this._24 * m._42;
    tmp._32 = this._31 * m._12 + this._32 * m._22 + this._33 * m._32 +this._34 * m._42;
    tmp._42 = this._41 * m._12 + this._42 * m._22 + this._43 * m._32 +this._44 * m._42;

    tmp._13 = this._11 * m._13 + this._12 * m._23 + this._13 * m._33 + this._14 * m._43;
    tmp._23 = this._21 * m._13 + this._22 * m._23 + this._23 * m._33 + this._24 * m._43;
    tmp._33 = this._31 * m._13 + this._32 * m._23 + this._33 * m._33 + this._34 * m._43;
    tmp._43 = this._41 * m._13 + this._42 * m._23 + this._43 * m._33 + this._44 * m._43;

    tmp._14 = this._11 * m._14 + this._12 * m._24 + this._13 * m._34 + this._14 * m._44;
    tmp._24 = this._21 * m._14 + this._22 * m._24 + this._23 * m._34 + this._24 * m._44;
    tmp._34 = this._31 * m._14 + this._32 * m._24 + this._33 * m._34 + this._34 * m._44;
    tmp._44 = this._41 * m._14 + this._42 * m._24 + this._43 * m._34 + this._44 * m._44;

    return tmp;
  }    
  VecMul(v){
    let tmp = new Vector3();
    tmp.x = this._11 * v.x + this._12 * v.y + this._13 * v.z + this._14;
    tmp.y = this._21 * v.x + this._22 * v.y + this._23 * v.z + this._24;
    tmp.z = this._31 * v.x + this._32 * v.y + this._33 * v.z + this._34;
        //projとの掛け算
    if (this._43 < 0) {//応用がきかないが、これでproj matと判断
  ...
  pers(z) {
    ...
    //float a = f / (-f + n);
    //float b = a * n;
    this._11 = 1/z;          this._12 = 0;        this._13 = 0;            this._14 = 0;
    this._21 = 0;            this._22 = 1/z;      this._23 = 0;            this._24 = 0;
    this._31 = 0;            this._32 = 0;        this._33 = 1;            this._34 = 0;
    this._41 = 0;            this._42 = 0;        this._43 = 0;            this._44 = 1;
  }
  camera(camPos,lookat,up) {
    //カメラのローカル軸座標を求める(正規直交ベクトル)
    let z = culVecNormalize(lookat.returnMinus(camPos));
    let x = culVecNormalize(culVecCross(up, z));
    let y = culVecCross(z, x);
    this._11 = x.x;    this._12 = x.y;    this._13 = x.z;    this._14 = x.x * -camPos.x + x.y * -camPos.y + x.z * -camPos.z;
    this._21 = y.x;    this._22 = y.y;    this._23 = y.z;    this._24 = y.x * -camPos.x + y.y * -camPos.y + y.z * -camPos.z;
    this._31 = z.x;    this._32 = z.y;    this._33 = z.z;    this._34 = z.x * -camPos.x + z.y * -camPos.y + z.z * -camPos.z;
    this._41 = 0;      this._42 = 0;      this._43 = 0;      this._44 = 1;
  }
}
// Camera
let camPos = new Vector3(1,-3,-0);
let lookat = new Vector3(0,0,0);
let up = new Vector3(0,1,0);
let CRot = new Vector3(0,0,0);
...
class moveObject{ 
  constructor(orgObject,polyList){
...
class Polygon{
  ...
  constructor(Pos1,Pos2,Pos3,UV,image){
    ...
    let Va = Pos1.returnMinus(Pos2);
    let Vb = Pos3.returnMinus(Pos1);
    this.crossVector = culVecNormalize(culVecCross(Va,Vb));
  }
  BufferRegister(renderZBuffer){
	  triangleToBuffer(renderZBuffer,1,this.image,
        [
         this.moveVertices[0].x, this.moveVertices[0].y,this.moveVertices[0].z,
         this.moveVertices[1].x, this.moveVertices[1].y,this.moveVertices[1].z,
      	 this.moveVertices[2].x, this.moveVertices[2].y,this.moveVertices[2].z,
        ],
        [
         this.UV[0], this.UV[1],
         this.UV[2], this.UV[3],
...
function culVecCross(ver1,ver2,ver3){
  let crossVector3 = new Vector3();
  crossVector3.x = (ver1.y-ver2.y)*(ver2.z-ver1.z)-(ver1.z-ver2.z)*(ver3.y-ver2.y);
  crossVector3.y = (ver1.z-ver2.z)*(ver3.x-ver2.x)-(ver1.x-ver2.x)*(ver3.z-ver2.z);
  crossVector3.z = (ver1.x-ver2.x)*(ver3.y-ver2.y)-(ver1.y-ver2.y)*(ver3.x-ver2.x);

  crossVector3 = culVecNormalize(crossVector3);

  return crossVector3;
}
*/
function culVecNormalize(vector3){
  let normalizeVector3 = new Vector3();
  let length = Math.sqrt(vector3.x * vector3.x + vector3.y * vector3.y + vector3.z * vector3.z);
  normalizeVector3.x =  vector3.x /= length;
  normalizeVector3.y =  vector3.y /= length;
  normalizeVector3.z =  vector3.z /= length;
...
let cubeVerts = [];
cubeVerts.push(new Vector3(-1/4,-1/4,-1/4));
cubeVerts.push(new Vector3(1/4,-1/4,-1/4));
cubeVerts.push(new Vector3(1/4,1/4,-1/4));
cubeVerts.push(new Vector3(-1/4,1/4,-1/4));
cubeVerts.push(new Vector3(-1/4,-1/4,1/4));
...
cubeImage.addEventListener("load", function() {
	...
	//cubes.push(new Object(orgCubeVerts,0.0,0.0,4.5,30,0,0,1,0,cubeFaceIndex,cubePlaneUV,cubeFaceUV,false,true,cubePixelImage));
  cubes1.push(new Object(cubeVerts,0.0,0.0,4.5,0,0,0,1,0,cubeFaceIndex,cubePlaneUV,cubeFaceUV,false,true,cubePixelImage));
  /*
...
viewMatrix.camera(camPos,lookat,up);
  ...
  for(let num =0;num<cubes1.length;num++){
    let worldVerts = [];
    let object = cubes1[num];
    worldMatrix.identity();
    //反対にするだけで前と同じになる
    worldMatrix.mulTranslate(object.centerObjX-Cx,object.centerObjY-Cy,object.centerObjZ-Cz);
    worldMatrix.mulRotateX(object.objRotX+CRotX);
    worldMatrix.mulRotateY(object.objRotY+CRotY);    
    worldMatrix.mulRotateZ(object.objRotZ+CRotZ); 
    worldMatrix.mulScaling(object.scale,object.scale,object.scale);
    for (var i = 0; i < object.verts.length; i++) {
      projectionMatrix.identity();
      worldVerts.push(worldMatrix.VecMul(object.verts[i]));
      projectionMatrix.pers(worldVerts[i].z)
      worldVerts[i] = projectionMatrix.VecMul(worldVerts[i]);
      let extensionVector = new Vector3(SCREEN_SIZE_W,SCREEN_SIZE_H,1);
      worldVerts[i].multiply(extensionVector);
      let offsetVector = new Vector3(SCREEN_SIZE_W/2,SCREEN_SIZE_H/2,0);
      worldVerts[i].plus(offsetVector);
      worldVerts[i].x = Math.floor(parseFloat(worldVerts[i].x) + 0.5);
      worldVerts[i].y = Math.floor(parseFloat(worldVerts[i].y) + 0.5);
    }
    let Poly = []
    for(let i=0;i<object.faceIndex.length;i++){
      let triangleIndex = object.faceIndex[i];
      ...
      if(object.faceUV.length == 0){
        ...
        let movedObject = new moveObject(object,Poly);
        moveObjects.push(movedObject);
    }
  }
	//cuberegister
...
for(let j=0;j<moveObjects.length;j++){
	for(let i=0;i<moveObjects[j].polygonNum;i++){
	  ...
	  if(moveObjects[j].orgObject.backCullingFlag == true){
	    if(moveObjects[j].polygonList[i].crossVector.z<0){
