...
class Object{
  ...
  constructor(verts,x,y,z,RotX,RotY,RotZ,numCorners,faceIndex,uv,backGroundFlag,backCullingFlag,img){
...
monkeysImage.addEventListener("load", function() {
	...
	let faceIndex = [];
	let uv = []
	monkeys.push(new Object(monkeyVerts,0.0,-1.0,4.5,-180,0,0,0,faceIndex,uv,false,false,monkeyPixelImage));
...
cubeImage.addEventListener("load", function() {
	...
	let faceIncex = [];
	let uv = []
	//cubes.push(new Object(orgCubeVerts,0.0,-1.0,4.5,-180,0,0,0,faceIndex,uv,false,false,cubePixelImage));
  ...
  for(let num =0;num<spheres.length;num++){
	...
	for(let num=0;num<monkeys.length;num++){
		...
		moveCubeInfo.backCullingFlag = cube.backCullingFlag;
			...
			let Poly = []
			for(let i=0;i<cube.faceIndex.length;i++){
				let triangleIndex = cube.faceIndex[i];
				let UV = [
			       cube.UV[triangleIndex.faceIndex[0]].u, cube.UV[triangleIndex.faceIndex[0]].v,
			       cube.UV[triangleIndex.faceIndex[1]].u, cube.UV[triangleIndex.faceIndex[1]].v,
			       cube.UV[triangleIndex.faceIndex[2]].u, cube.UV[triangleIndex.faceIndex[2]].v,
			      ]
