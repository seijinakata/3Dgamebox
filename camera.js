...
let cube1 = new Object(orgCubeVerts,0,0,5,0,0,0,false,true,'https://cdn.glitch.com/6efb404c-071d-4250-b26b-2bdeba7c6ed2%2F0473fc3d55a213710b0b024a2fdc3a51.jpg?v=1620968464541');
let cube2 = new Object(orgCubeVerts,-0,-1,4.6,0,0,0,false,true,'https://cdn.glitch.com/6efb404c-071d-4250-b26b-2bdeba7c6ed2%2F0473fc3d55a213710b0b024a2fdc3a51.jpg?v=1620968464541');
let cube3 = new Object(orgCubeVerts,0,-2.5,5.0,0,0,0,false,true,'https://cdn.glitch.com/6efb404c-071d-4250-b26b-2bdeba7c6ed2%2F0473fc3d55a213710b0b024a2fdc3a51.jpg?v=1620968464541');
let cube4 = new Object(orgCubeVerts,0,-3,5,0,0,0,false,true,'https://cdn.glitch.com/6efb404c-071d-4250-b26b-2bdeba7c6ed2%2F0473fc3d55a213710b0b024a2fdc3a51.jpg?v=1620968464541');
...
function vertsCulMaxMin(moveVerts){
  let maxminObject = [];
  ...
  for(let j=1;j<cubes.length;j++){
    ...
    for(let i=0;i<moveObjects.length;i++){
      if(object.centerObjY<moveObjects[i].centerObjY){
        ...
        if(object.preGravityCollision == null){
          if(i>=moveObjects.length-1){
            object.preGravityCollision = false;
            break;
          }
          continue;
        }
   ...
   if(object.preGravityCollision == false){
      cubes[j].centerObjY += 0.01;
   } 
  }

    /*
  for(let i=1;i<cubes.length;i++){
        moveObjects[i].preGravityCollision = AABBcollision(moveObjects[i-1],moveObjects[i]);
    if(!moveObjects[i].preGravityCollision){
      cubes[i].centerObjY += 0.01;
    }else{
    let upIndex = 0;
    let downIndex = 0;
      if(moveObjects[i-1].centerObjY<moveObjects[i].centerObjY){
        upIndex = i-1;
        downIndex = i;        
      }else{
        upIndex = i;
    ...
    let upObjectY = Math.floor(moveObjects[upIndex].centerObjY*10)/10;
    let projectionList  = setProjectionMatrix(cubes[upIndex],Cz,Cx,upObjectY,CRotZ,CRotX,CRotY);
    ...
    CRz = projectionList.CRz;
    distance_from_Obj_to_Camera = projectionList.distance_from_Obj_to_Camera;
    let moveCubeInfo = cameraModel(cubes[upIndex],distance_from_Obj_to_Camera,ORz,ORy,ORx,CRz,CRy,CRx,Extension,offset);
      
    let maxminObject = vertsCulMaxMin(moveCubeInfo);
    moveCubeInfo.maxX = maxminObject.maxX;
    moveCubeInfo.minX = maxminObject.minX;
    moveCubeInfo.maxY = maxminObject.maxY;
    moveCubeInfo.minY = maxminObject.minY;  
    ...
    projectionList = setProjectionMatrix(cubes[downIndex],Cz,Cx,upObjectY,CRotZ,CRotX,CRotY);
    ...
    CRz = projectionList.CRz;
    distance_from_Obj_to_Camera = projectionList.distance_from_Obj_to_Camera;
    moveCubeInfo = cameraModel(cubes[downIndex],distance_from_Obj_to_Camera,ORz,ORy,ORx,CRz,CRy,CRx,Extension,offset);
