...
// 面クラス、三角形の結び順格納
class Face {
    constructor(v0,v1,v2,c,uv0,uv1,uv2) {
        this.v.push(v0);
        this.v.push(v1);
        this.v.push(v2);
        this.color = c;
...
class ModelLoadData{
    ...
    JSONLoader(file, callback) {
        ...
        x.onreadystatechange = () => {
            if (x.readyState == 4) {
                this.json = JSON.parse(JSON.stringify(x.responseText));
                // 読込完了コールバック
                callback();
            }
        }
...
model1.JSONLoader("cube.js", (() => model1.onJSONLoaded()));
class Object{
...
cubeImage.addEventListener("load", function() {
	...
	cubes.push(new Object(orgCubeVerts,0.0,-0.33,3,0,0,0,0,false,true,cubePixelImage));
...
if( skyPixelImage.length == 0  || cubePixelImage.length == 0 ||
	roadPixelImage.length == 0 || sandPixelImage.length == 0 || model1.getJSON() == undefined){
	ctx.font = '50pt Arial';
 	ctx.fillStyle = 'rgba(0, 0, 255)';
 	ctx.fillText("now loding", SCREEN_SIZE_W/2, SCREEN_SIZE_H/2);
	return 0;
}
console.log(model1.getJSON());
  let cameraRotMatrix = [];
