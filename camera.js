...
import {matIdentity,mulMatTranslate,mulMatScaling, matMul,matVecMul,matPers,matCamera,mulMatRotateX,mulMatRotateY,mulMatRotateZ, getInverseMatrix, matRound4X4, protMatVecMul} from './matrix.js';
    ...
    JSONLoader(file, callback) {
   	...
   	onJSONLoaded() {
   		...
   		for(let i=0;i<this.json.data.index.array.length;i+=triangleIndex){
   			...
   			let tempFaceInde = setFaceIndex(v1,v2,v3);
...
class Polygon{
  ...
  constructor(Pos1,Pos2,Pos3,UV,image){
    this.moveVertices =   [[0,0,0],
                          [0,0,0],
                          [0,0,0]];
    this.moveVertices[0][0] = Pos1[0];
    this.moveVertices[0][1] = Pos1[1];
    this.moveVertices[0][2] = Pos1[2];

    this.moveVertices[1][0] = Pos2[0];
    this.moveVertices[1][1] = Pos2[1];
    this.moveVertices[1][2] = Pos2[2];

    this.moveVertices[2][0] = Pos3[0];
    this.moveVertices[2][1] = Pos3[1];
    this.moveVertices[2][2] = Pos3[2];
    this.image = image;
    this.UV = UV;
...
function objectPolygonPush(objects,objectNumber,moveObjects,viewMatrix){
  ...
  for (var i = 0; i < object.verts.length; i++) {
    ...
    let projectionMatrix =  matPers(worldVerts[i][2]);
    protMatVecMul(projectionMatrix,worldVerts[i]);
  ...
  for(let i=0;i<object.faceIndex.length;i++){
    ...
    Poly.push(new Polygon(worldVerts[triangleFaceIndex[0]],worldVerts[triangleFaceIndex[1]],worldVerts[triangleFaceIndex[2]],UV,object.image));
  }
...
for(let j=0;j<moveObjects.length;j++){
	for(let i=0;i<moveObjects[j].polygonNum;i++){
	  ...
	  if(moveObjects[j].orgObject.backCullingFlag == true){
       ...
       triangleToBuffer(shadowMap,null,inverseViewMatrix,inverseViewPortMatrix,sunViewMatrix,viewPortMatrix,1,moveObjects[j].polygonList[i].image,moveObjects[j].polygonList[i].moveVertices,
          [
...
for(let j=0;j<moveObjects.length;j++){
	for(let i=0;i<moveObjects[j].polygonNum;i++){
	  ...
	  if(moveObjects[j].orgObject.backCullingFlag == true){
	    if(moveObjects[j].polygonList[i].crossVector3[2]<0){
        triangleToBuffer(zBuffering,shadowMap,inverseViewMatrix,inverseViewPortMatrix,sunViewMatrix,viewPortMatrix,1,moveObjects[j].polygonList[i].image,moveObjects[j].polygonList[i].moveVertices,
          [
            moveObjects[j].polygonList[i].UV[0], moveObjects[j].polygonList[i].UV[1],
            moveObjects[j].polygonList[i].UV[2], moveObjects[j].polygonList[i].UV[3],
            moveObjects[j].polygonList[i].UV[4], moveObjects[j].polygonList[i].UV[5]
