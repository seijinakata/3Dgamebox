...
cubeImage.addEventListener("load", function() {
  ...
  cubes1.push(new Object(cubeVerts,0.0,0.0,4.5,30,0,0,1,0,cubeFaceIndex,cubePlaneUV,cubeFaceUV,false,true,cubePixelImage));
  /*
...
if( skyPixelImage.length == 0  || cubePixelImage.length == 0 ||
  ...
  for(let num =0;num<cubes1.length;num++){
    let worldVerts = [];
    let currentCube1 = cubes1[num];
    worldMatrix.identity();
    worldMatrix.mulTranslate(currentCube1.centerObjX,currentCube1.centerObjY,currentCube1.centerObjZ);
    worldMatrix.mulRotateX(currentCube1.objRotX);
    worldMatrix.mulRotateY(currentCube1.objRotY);    
    worldMatrix.mulRotateZ(currentCube1.objRotZ);
    worldMatrix.mulScaling(currentCube1.scale,currentCube1.scale,currentCube1.scale);
    for (var i = 0; i < currentCube1.verts.length; i++) {
      projectionMatrix.identity();
      worldVerts.push(worldMatrix.VecMul(currentCube1.verts[i]));
