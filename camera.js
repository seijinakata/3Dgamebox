...
if(catPixelImage.length == 0 || skyPixelImage.length == 0  || cubePixelImage.length == 0 ||
...
for(let i=0;i<moveObjects[0].polygonNum;i++){
  //-の方がこちらに近くなる座標軸だから
  if(moveObjects[0].backCullingFlag == true){
    if(moveObjects[0].polygonList[i].crossZ<0){
		let objectZ = moveObjects[0].centerObjZ;
	  	moveObjects[0].polygonList[i].draw(renderZBuffer,objectZ);
    } 
  }else{
      	let objectZ = moveObjects[0].centerObjZ;
	  	moveObjects[0].polygonList[i].draw(renderZBuffer,objectZ);
