...
class Polygon{
  ...
  BufferRegister(zBuffering,shadowMap,inverseViewMatrix,inverseViewPortMatrix,sunViewMatrix,viewPortMatrix){
	triangleToBuffer(zBuffering,shadowMap,inverseViewMatrix,inverseViewPortMatrix,sunViewMatrix,viewPortMatrix,1,this.image,this.moveVertices,
    [
      this.UV[0], this.UV[1],
      this.UV[2], this.UV[3],
      this.UV[4], this.UV[5]
...
for(let j=0;j<moveObjects.length;j++){
	for(let i=0;i<moveObjects[j].polygonNum;i++){
	  ...
	  if(moveObjects[j].orgObject.backCullingFlag == true){
	    if(moveObjects[j].polygonList[i].crossVector3[2]<0){
		  moveObjects[j].polygonList[i].BufferRegister(zBuffering,shadowMap,inverseViewMatrix,inverseViewPortMatrix,sunViewMatrix,viewPortMatrix);
	    } 
	  }else{
		  moveObjects[j].polygonList[i].BufferRegister(zBuffering,shadowMap,inverseViewMatrix,inverseViewPortMatrix,sunViewMatrix,viewPortMatrix);
	  }
