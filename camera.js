...
import {renderBuffer,pixel,bufferPixelInit,bufferInit,pictureToPixelMap,dotPaint,dotLineBufferRegister,triangleRasterize,textureTransform,triangleToBuffer,sort_index,branch} from './paint.mjs';
...
function objectPolygonPush(objects,objectNumber,moveObjects,viewMatrix){
  ...
  for(let i=0;i<object.faceIndex.length;i++){
    ...
    let tempverts = JSON.parse(JSON.stringify(Poly[i].moveVertices));
    sort_index(tempverts,1);
    let pt = tempverts[0];
    let pm = tempverts[1];
    let pb = tempverts[2];
    let mid = pm[1];
    let midpos = branch(pt,pb,mid);
    console.log(midpos)
...
cubeImage.addEventListener("load", function() {
	...
	cubes.push(new Object(orgCubeVerts,-0.0,-0.55,1.5,0,60,0,1,1,1,0,cubeFaceIndex,cubePlaneUV,cubeFaceUV,false,false,cubePixelImage));
  /*
