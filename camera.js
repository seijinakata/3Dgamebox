...
// Camera
let cameraPos = setVector3(0,-1,-3);
let lookat = setVector3(0.0,-0.0,1);
let sunPos = setVector3(0,-4,-3);
...
for(let j=0;j<moveObjects.length;j++){
	for(let i=0;i<moveObjects[j].polygonNum;i++){
	  ...
	  if(moveObjects[j].orgObject.backCullingFlag == true){
       ...
       triangleToBuffer(shadowMap,null,1,moveObjects[j].polygonList[i].image,moveObjects[j].polygonList[i].moveVertices,moveObjects[j].polygonList[i].crossWorldVector3,
          [
      ...
      let getPixel = zBuffering[j][i][0];
          //シャドウマップ
      		//camera
                ...
                //console.log(zBuffering[j][i][0].crossWorldVector3)
