...
import {rgPlaneVerts,rgCubeVerts,orgPlaneVerts, orgCubeVerts} from './orgverts.mjs';
...
function vertsCulAABBMaxMinCenter(orgObject){

  let cubeVerts1Mat = translationMatrix(orgObject.centerObjX,orgObject.centerObjY,orgObject.centerObjZ);
  let objectOrgVerts = [];
...
cubeImage.addEventListener("load", function() {
	...
	cubes.push(new Object(orgCubeVerts,0.0,-0.6,3,0,0,0,0,false,true,cubePixelImage));
	cubes.push(new Object(orgCubeVerts,0.0,0.0,3,0,0,0,0,false,true,cubePixelImage));
...
roadImage.addEventListener("load", function() {
	...
	for(let j=0;j<2;j++){
  		for(let i=0;i<9;i++){
    		//planes.push(new Object(orgPlaneVerts,-0.5+1.0*j,0,1.5+1.0*i,-90,0,0,0,false,false,roadPixelImage));
  		}
	}
}, true);
...
groundImage.addEventListener("load", function() {
	...
	for(let j=0;j<3;j++){
	  for(let i=0;i<9;i++){
	    //planes.push(new Object(orgPlaneVerts,-1.5-1.0*j,0,1.5+1.0*i,-90,0,0,0,false,false,sandPixelImage));
	  }
	}
	for(let j=0;j<3;j++){
	  for(let i=0;i<9;i++){
...
if( skyPixelImage.length == 0  || cubePixelImage.length == 0 ||
	roadPixelImage.length == 0 || sandPixelImage.length == 0){
	ctx.font = '50pt Arial';
  ...
  for(let num =0;num<spheres.length;num++){
    ...
    let maxZ = 0;
    let minZ = 0;
    let maxminCenterObject = vertsCulAABBMaxMinCenter(moveCameraSphere[1]);
  ...
  for(let num=0;num<cubes.length;num++){
    ...
    let maxZ = 0;
    let minZ = 0;
    let maxminCenterObject = vertsCulAABBMaxMinCenter(cube);
    console.log(maxminCenterObject)
  ...
  for(let num=0;num<planes.length;num++){
    ...
    let maxZ = 0;
    let minZ = 0;
    let maxminCenterObject = vertsCulAABBMaxMinCenter(plane);
  ...
  /*
    for(let i=spheres.length;i<spheres.length+cubes.length;i++){	
    	for(let j = spheres.length;j<spheres.length+cubes.length+planes.length;j++){
    		if(i != j){
    		    if(AABBcollision(moveObjects[i],moveObjects[j]) == true && moveObjects[i].centerObjY<moveObjects[j].centerObjY){
    		       moveObjects[i].gravityCollision = true;
             	   break;    		          
    		    }
    		}
    	} 
    if(moveObjects[i].gravityCollision == false){
      moveObjects[0].orgObject.centerObjY += 0.01;
   } 
  }
  */
  if(AABBcollision(moveObjects[0],moveObjects[1]) == false){
  	moveObjects[0].orgObject.centerObjY += 0.01;
  }
  /*  
