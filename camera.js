...
import {renderBuffer,pixel,bufferPixelInit,bufferInit,pictureToPixelMap,dotPaint,dotLineBufferRegister,triangleRasterize,textureTransform} from './paint.mjs';
...
let triangle = bufferPixelInit(SCREEN_SIZE_H,SCREEN_SIZE_W);

let triangleFrame = new Array(SCREEN_SIZE_H);
for(let j=0;j<SCREEN_SIZE_H;j++){
	triangleFrame[j] = new pixel();
	triangleFrame[j].setPixel(-1,j,-1,-1,-1,-1,-1);
}

dotLineBufferRegister(triangleVerts[0][0],triangleVerts[1][0],triangleVerts[0][1],triangleVerts[1][1],0,255,255,255,triangleFrame);//0から1

dotLineBufferRegister(triangleVerts[1][0],triangleVerts[2][0],triangleVerts[1][1],triangleVerts[2][1],0,255,255,255,triangleFrame);//1から3
dotLineBufferRegister(triangleVerts[2][0],triangleVerts[0][0],triangleVerts[2][1],triangleVerts[0][1],0,255,255,255,triangleFrame);//3から0

triangleRasterize(triangle,triangleFrame,1,255,0,0,255,SCREEN_SIZE_H,SCREEN_SIZE_W);

//lengthが高さ、length[0]が横

let affinedTexture = textureTransform(1,-0.3,1,0.7,150,-350,catPixelImage,SCREEN_SIZE_H,SCREEN_SIZE_W);

for(let j=0;j<SCREEN_SIZE_H;j++){
	for(let i=0;i<SCREEN_SIZE_W;i++){
	let getPixel = affinedTexture[j][i].getPixel();
		if(affinedTexture[j][i].z>-1){
			if(getPixel.z>-1){
...
for(let j=0;j<SCREEN_SIZE_H;j++){
	for(let i=0;i<SCREEN_SIZE_W;i++){
		let getPixel = triangle[j][i].getPixel();
		if(getPixel.z>-1){
			//dotPaint(j,i,getPixel.r,getPixel.g,getPixel.b,getPixel.a,ctx)
		}
	}
...
//ctx.save();
//ctx.transform(-1,0.3,1,0.7,SCREEN_SIZE_W/2,SCREEN_SIZE_H/2-catPixelImage.length/2);
...
for(let j=0;j<SCREEN_SIZE_H;j++){
	for(let i=0;i<SCREEN_SIZE_W;i++){
		if(triangle[j][i].z>-1 && affinedTexture[j][i].z>-1){
			renderZBuffer[j][i].insert(affinedTexture[j][i]);
		}
	}
}
//レンダリングバッファ作画
