...
function projBranch(a,b,Y){
    let  t = (Y-a.projY)/(b.projY-a.projY);
    return setVector3(a.projX*(1-t)+b.projX*t,Y,a[2]*(1-t)+b[2]*t);
}
...
function scan_vertical(buffer,screen_size_h,screen_size_w,pt,pm,pb,projPt,projPm,projPb,a,b,c,d,h,w,imageData,
	...
	//viewport前は0から1000で管理三桁で四捨五入0.5は画面の中央
    let triangleTop1 = top_int((projPt[0]+0.5)*1000);
	let triangleTop2 = top_int((projPt[1]+0.5)*1000);

    let triangleBtm1 = top_int((projPb[1]+0.5)*1000);
    let triangleTop = pt[1];
    let triangleBtm = pb[1];
	console.log(triangleTop1)
	console.log(triangleBtm1)
    ...
    let mid = pm[1];
	let viewMid = matVecMul(viewPortMatrix,projPm);

    let pl = [],pr = [];
    let tmp = branch(pt,pb,mid);//pt->
	let projTmp = branch(projPt,projPb,projPm[1]);//pt->
	let proj = matVecMul(viewPortMatrix,projTmp);
