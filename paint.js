...
function scan_horizontal(zBuffering,screen_size_w,y,se,iA,h,w,imageData,shadowMap,crossWorldVector3){
...
export function triangleToBuffer(zBuffering,shadowMap,textureAlpha,img,vertex_list,crossWorldVector3,uv_list)
{
  //各点のZ座標がこれより下なら作画しない。
  if (vertex_list[0][2] > 0.0 && vertex_list[1][2]> 0.0 && vertex_list[2][2] > 0.0) {
	...
	*/
	...
	if(shadowMap != null){
		...
		iA[2] = - b / det;
		iA[3] = a / det;
		h = vertex_list[0][1] - (b * uv_list[0] * img.width + d * uv_list[1] * img.height);
	...
	let pb = tempverts[2];
	 ...
	 scan_vertical(zBuffering,SCREEN_SIZE_H,SCREEN_SIZE_W,pt,pm,pb,iA,h,w,img,
		shadowMap,crossWorldVector3);
 	/*
