import { matVecMul,matIdentity,matPers,getInverseMatrix, matMul } from "./matrix.js";
import { setVector2,setVector3, vec2Minus, vec2Plus, vecMinus, vecMul } from "./vector.js";
...
function scan_vertical(buffer,screen_size_h,screen_size_w,pt,pm,pb,projPt,projPm,projPb,a,b,c,d,h,w,imageData,
	...
	let projTmpBranchPoint = branch(projPt,projPb,projPm[1]);//pt->
	let projBranchPoint = [(projTmpBranchPoint[0]+0.5)*1000,top_int((projTmpBranchPoint[1]+0.5)*1000),projTmpBranchPoint[2]];
	let proj = matVecMul(viewPortMatrix,projTmpBranchPoint);
	let projBranchP = [(projBranchPoint[0]/1000),(projBranchPoint[1]/1000),projBranchPoint[2]];

	let proj1 = matVecMul(viewPortMatrix1,projBranchP);
	let projBranch = [proj1[0],top_int(proj1[1]),proj1[2]];

	projPt[1] = top_int((projPt[1]+0.5)*1000);
	projPb[1] = top_int((projPb[1]+0.5)*1000);

    let triangleTop = pt[1];
    let triangleBtm = pb[1];

	let triangleTop1 = projPt[1];
    let triangleBtm1 = projPb[1];
	...
	if(triangleTop<0)triangleTop=0;
    if(screenHeight<triangleBtm)triangleBtm=screenHeight;
	*/
	...
	/*if(projTmpBranchPoint[0]<projPm[0]){
        pl = projTmpBranchPoint;
        pr = projPm;
    }else{
        pl = projPm;
        pr = projTmpBranchPoint;           
    }
    let m = projBranchPoint[1];*/
    let m = mid;
    if(m<0)m=0;
    ...
    if(triangleTop<m){//upper 
        ...
        do{
            ...
            se.min = sl;
            se.max = sr;
            scan_horizontal(buffer,screen_size_w,y,se,a,b,c,d,h,w,
    ...
    if(m<triangleBtm){//lower
        ...
        do{
            ...
            se.max = sr;
			//console.log(se.max)
            scan_horizontal(buffer,screen_size_w,y,se,a,b,c,d,h,w,imageData,
				shadowMap,inverseViewMatrix,inverseViewPortMatrix,sunViewMatrix,viewPortMatrix);
            sl = vec2Plus(sl,dl);//
            sr = vec2Plus(sr,dr);//
...
function scan_horizontal(buffer,screen_size_w,y,se,a,b,c,d,h,w,imageData,shadowMap,inverseViewMatrix,inverseViewPortMatrix,sunViewMatrix,viewPortMatrix){
				...
				if(shadowMap !=null){
					...
					if(pixelVector3[0]>0 && pixelVector3[0]<SCREEN_SIZE_W){
						if(pixelVector3[1]>0 && pixelVector3[1]<SCREEN_SIZE_H){
							if(shadowMap[pixelVector3[1]][pixelVector3[0]].getZ()+0.116<pixelVector3[2]){
					...
					buffer[y][x].insert(affinedPixel);
					buffer[y][x].setZ(sz);
				}else{
					buffer[y][x].setZ(sz);
				}
...
export function triangleToBuffer(renderZBuffer,shadowMap,inverseViewMatrix,inverseViewPortMatrix,sunViewMatrix,viewPortMatrix,textureAlpha,img,vertex_list,uv_list)
{
  //各点のZ座標がこれより下なら作画しない。
  if (vertex_list[0][2] > 0.0 && vertex_list[1][2]> 0.0 && vertex_list[2][2] > 0.0) {
	...
	let projVerts = [];
	for(let i=0;i<3;i++){
		let projTempVerts = [vertex_list[i].projX,vertex_list[i].projY,vertex_list[i][2]];
		projVerts.push(projTempVerts);

	}
	 sort_index(tempverts,1);//ys
	 sort_index(projVerts,1);//ys
	 let projPt = projVerts[0];
	 let pt = tempverts[0];
	 let projPm = projVerts[1];
	 let pm = tempverts[1];
	 let projPb = projVerts[2];
	 let pb = tempverts[2];

	 scan_vertical(renderZBuffer,SCREEN_SIZE_H,SCREEN_SIZE_W,pt,pm,pb,projPt,projPm,projPb,a,b,c,d,vertex_list[0][1]
