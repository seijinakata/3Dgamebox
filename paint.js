	...
	for(let j=minY;j<maxY;j++){
	 ...
	 if(endX > screen_size_w-1){
		...
		for(let start = startX ;start<=endX;start++){
			buffer[j][start].setPixel(z,r,g,b,a);
		}
	}
}
    ...
    if(l<r){
        ...
        do{
            ...
            if(z>sz){
				...
				affinedPixel.setPixel(sz,orgTexture.data[index],orgTexture.data[index + 1],orgTexture.data[index + 2],orgTexture.data[index + 3]);
				if(shadowMap !=null){
					//camera
					...
					if(pixelVector3[0]>0 && pixelVector3[0]<SCREEN_SIZE_W){
						if(pixelVector3[1]>0 && pixelVector3[1]<SCREEN_SIZE_H){
							if(shadowMap[pixelVector3[1]][pixelVector3[0]].get().getPixel().z+0.116<pixelVector3[2]){
		...
		//その列の始点終点が画面内にあるがその相方がが飛び出している場合
	     ...
	     for(affinedX = currentMinX; affinedX <= currentMaxX; affinedX++){
			...
			if(renderZBuffer[affinedY][affinedX].get().getPixel().z>=pixelZ){
				let index = (orgx + orgy * orgTexture.width) * 4;
				let affinedPixel = new pixel();
				affinedPixel.setPixel(pixelZ,orgTexture.data[index],orgTexture.data[index + 1],orgTexture.data[index + 2],orgTexture.data[index + 3]);
				if(shadowMap !=null){
					//camera
