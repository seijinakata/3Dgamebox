...
import { matVecMul,matIdentity,matPers,getInverseMatrix, matMul, CalInvMat4x4 } from "./matrix.js";
...
function getInvert(_11,_12,_21,_22){
    var out = [[1,0],
				[0,1]]
    //逆行列の公式 ad - bc の部分
...
function scan_vertical(zBuffering,screen_size_h,screen_size_w,pt,pm,pb,a,b,c,d,h,w,imageData,
    ...
    if(triangleTop<m){//upper 
        ...
        do{
            ...
            se.min = sl;
            se.max = sr;
            scan_horizontal(zBuffering,screen_size_w,y,se,a,b,c,d,h,w,
    ...
    if(m<triangleBtm){//lower
        ...
        do{
            ...
            se.max = sr;
			//console.log(se.max)
            scan_horizontal(zBuffering,screen_size_w,y,se,a,b,c,d,h,w,imageData,
				shadowMap,inverseViewMatrix,inverseViewPortMatrix,sunViewMatrix,viewPortMatrix);
            sl = vec2Plus(sl,dl);//
            sr = vec2Plus(sr,dr);//
...
function scan_horizontal(zBuffering,screen_size_w,y,se,a,b,c,d,h,w,imageData,shadowMap,inverseViewMatrix,inverseViewPortMatrix,sunViewMatrix,viewPortMatrix){
				...
				if(shadowMap !=null){
					...
					if(pixelVector3[0]>0 && pixelVector3[0]<SCREEN_SIZE_W){
						if(pixelVector3[1]>0 && pixelVector3[1]<SCREEN_SIZE_H){
							if(shadowMap[pixelVector3[1]][pixelVector3[0]][0].z+0.1<pixelVector3[2]){
  ...
  if (vertex_list[0][2] > 0.0 && vertex_list[1][2]> 0.0 && vertex_list[2][2] > 0.0) {
	...
	*/
	...
	let pb = tempverts[2];
	 ...
	 scan_vertical(zBuffering,SCREEN_SIZE_H,SCREEN_SIZE_W,pt,pm,pb,a,b,c,d,vertex_list[0][1]
