...
function scan_vertical(buffer,screen_size_h,screen_size_w,pt,pm,pb){
    let t = top_int(pt[1]);
    let b = top_int(pb[1]);
    ...
    if(t<m){//upper 
        ...
        do{
            ...
            scan_horizontal(buffer,screen_size_w,y,se);
            sl = vec2Plus(sl,dl);//
            sr = vec2Plus(sr,dr);//
...
function scan_horizontal(buffer,screen_size_w,y,se){
    let l,r;
    l = top_int(se.min[0]);
    r = top_int(se.max[0]);
    ...
    if(l<r){
        ...
        do{
            ...
            if(z>sz){
				...
				affinedPixel.setPixel(y,x,sz,255,255,255,255);
...
export function triangleToBuffer(renderZBuffer,shadowMap,inverseViewMatrix,inverseViewPortMatrix,sunViewMatrix,viewPortMatrix,textureAlpha,img,vertex_list,uv_list)
{
 ...
 /*
	 ...
	 */
	 ...
	 let pm = tempverts[1];
	 let pb = tempverts[2];
	 scan_vertical(renderZBuffer,SCREEN_SIZE_H,SCREEN_SIZE_W,pt,pm,pb);
		/*
    //平行移動を追加済み        
    g.transform(a, b, c, d,
