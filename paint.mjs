...
export function textureTransform(a,b,c,d,h,w,alpha,imageData,screen_size_h,screen_size_w,clipMinY,clipMaxY,bufferFrame,triangleZ){
	     ...
	     for(affinedX = currentMinX; affinedX < currentMaxX; affinedX++){
		      ...
		      if(orgy >=  orgTexture.length || orgy < 0){
		        continue;
			      ...
			      affinedTexture[affinedY][affinedX].z = triangleZ;
	     }
	}
...
export function triangleToBuffer(renderZBuffer,triangleZ,textureAlpha,img, vertex_list, uv_list)
{
  ...
  if (vertex_list[6] > 0.0 && vertex_list[7] > 0.0 && vertex_list[8] > 0.0) {
	...
	let affinedTexture = textureTransform(a,b,c,d,vertex_list[1]
	 - (b * uv_list[0] * img[0].length + d * uv_list[1] * img.length), vertex_list[0] - (a * uv_list[0] * img[0].length + c * uv_list[1] * img.length),textureAlpha,img,SCREEN_SIZE_H,SCREEN_SIZE_W,minY,maxY,triangleFrame,triangleZ);

	//バッファに登録
	for(let j=0;j<SCREEN_SIZE_H;j++){
		for(let i=0;i<SCREEN_SIZE_W;i++){
			if(affinedTexture[j][i].z < 99999){
				renderZBuffer[j][i].insert(affinedTexture[j][i],affinedTexture[j][i].z);
			}
		}
	}
