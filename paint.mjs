...
import { setVector2,setVector3, vec2Minus, vec2Plus, vecMinus } from "./vector.js";
...
function scan_vertical(buffer,screen_size_h,screen_size_w,pt,pm,pb,a,b,c,d,h,w,imageData,
	shadowMap,inverseViewMatrix,inverseViewPortMatrix,sunViewMatrix,viewPortMatrix){
    let triangleTop = top_int(pt[1]);
    let triangleBtm = top_int(pb[1]);
    ...
    if(triangleTop<m){//upper 
        ...
        let dr = delta_xz(er);
        //
        let gap_y = (triangleTop+0.5)-pt[1];
        //
        //start position
        let sl = vec2Plus(setVector2(pt[0],pt[2]),setVector2(dl[0]*gap_y,dl[1]*gap_y));
        let sr = vec2Plus(setVector2(pt[0],pt[2]),setVector2(dr[0]*gap_y,dr[1]*gap_y));
        let y=triangleTop;
        do{
			//Y座標ごとの切片
            let se = [];
    ...
    if(m<triangleBtm){//lower
        ...
        //
        //start position
        let sl = vec2Plus(setVector2(pl[0],pl[2]),setVector2(dl[0]*gap_y,dl[1]*gap_y));
        let sr = vec2Plus(setVector2(pr[0],pr[2]),setVector2(dr[0]*gap_y,dr[1]*gap_y));
...
function scan_horizontal(buffer,screen_size_w,y,se,a,b,c,d,h,w,imageData,shadowMap,inverseViewMatrix,inverseViewPortMatrix,sunViewMatrix,viewPortMatrix){
    ...
    if(l<r){
        ...
        let sz = se.min[1]+dz*gap_x;

        let x = l;
        do{
            let z = buffer[y][x].get().getPixel().z;
            if(z>sz){
				/* 元画像における縦方向座標を計算 */
				...
				affinedPixel.setPixel(y,x,sz,orgTexture.data[index],orgTexture.data[index + 1],orgTexture.data[index + 2],orgTexture.data[index + 3]);
				if(shadowMap !=null){
					//camera
					...
					if(pixelVector3[0]>0 && pixelVector3[0]<SCREEN_SIZE_W){
						if(pixelVector3[1]>0 && pixelVector3[1]<SCREEN_SIZE_H){
							if(shadowMap[pixelVector3[1]][pixelVector3[0]].get().getPixel().z+0.15<pixelVector3[2]){
					...
					buffer[y][x].insert(affinedPixel,affinedPixel.z);
				}else{
					buffer[y][x].insert(affinedPixel,affinedPixel.z);
				}
			}
...
export function triangleToBuffer(renderZBuffer,shadowMap,inverseViewMatrix,inverseViewPortMatrix,sunViewMatrix,viewPortMatrix,textureAlpha,img,vertex_list,uv_list)
{
  ...
  if (vertex_list[0][2] > 0.0 && vertex_list[1][2]> 0.0 && vertex_list[2][2] > 0.0) {
	...
	let tempverts = JSON.parse(JSON.stringify(vertex_list));
	 ...
	 let pm = tempverts[1];
	 let pb = tempverts[2];
	 scan_vertical(renderZBuffer,SCREEN_SIZE_H,SCREEN_SIZE_W,pt,pm,pb,a,b,c,d,vertex_list[0][1]
