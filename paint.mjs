...
triangleRasterize(triangle,triangleFrame,triangleZ,255,0,0,255,SCREEN_SIZE_H,SCREEN_SIZE_W);

let affinedTexture = textureTransform(a,b,c,d,vertex_list[1]
 - (b * uv_list[0] * img[0].length + d * uv_list[1] * img.length), vertex_list[0] - (a * uv_list[0] * img[0].length + c * uv_list[1] * img.length),textureAlpha,img,SCREEN_SIZE_H,SCREEN_SIZE_W);

//バッファに登録
for(let j=0;j<SCREEN_SIZE_H;j++){
	for(let i=0;i<SCREEN_SIZE_W;i++){
		if(triangle[j][i].z < 99999 && affinedTexture[j][i].z < 99999){
			renderZBuffer[j][i].insert(affinedTexture[j][i],triangle[j][i].z);
		}
	}
}
