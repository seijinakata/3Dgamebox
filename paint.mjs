...
export function dotLineBufferRegister(Xs,Xe,Ys,Ye,Zs,Ze,buffer){
	...
	if(Ye == Ys){
		if((Ys>=0 && Ys<SCREEN_SIZE_H) && (Ye>=0 && Ye<SCREEN_SIZE_H)){
			if(Xs>Xe){
				buffer[Ys].setMaxX(Xs);
				buffer[Ys].setMaxZ(Zs);
				buffer[Ys].setMinX(Xe);
				buffer[Ys].setMinZ(Ze);
			}else{
				buffer[Ys].setMaxX(Xe);
				buffer[Ys].setMaxZ(Ze);
				buffer[Ys].setMinX(Xs);
				buffer[Ys].setMinZ(Zs);
			}
		}
		return;
	}
